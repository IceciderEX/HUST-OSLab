...
static volatile int s_start_counter = 0;
//
// s_start: S-mode entry point of riscv-pke OS kernel.
// 在通过s_start进入S mode的时候，pke会对物理内存和内核页表进行初始化。
...
int s_start(void) {
  ...
  // 初始化完毕后所有核才能够开启页表，继续执行之后的指令。
  sync_barrier(&s_start_counter, NCPU);
